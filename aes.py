...
class AES:
    ...
    def __mix_single_column(self, *col: list[int]):
        ...
        c = col[0] ^ col[1] ^ gmul(col[2],0x02) ^ gmul(col[3],0x03)
        d = gmul(col[0],0x03) ^ col[1] ^ col[2] ^ gmul(col[3],0x02)
        return a, b, c, d
    ...
    def __mix_single_column_inv(self, *col: list[int]):
       ...
       b = gmul(col[0],0x09) ^ gmul(col[1],0x0e) ^ gmul(col[2],0x0b) ^ gmul(col[3],0x0d)
       c = gmul(col[0],0x0d) ^ gmul(col[1],0x09) ^ gmul(col[2],0x0e) ^ gmul(col[3],0x0b)
       d = gmul(col[0],0x0b) ^ gmul(col[1],0x0d) ^ gmul(col[2],0x09) ^ gmul(col[3],0x0e)
    ...
    def __round_decrypt(self, state_matrix, round_key_matrix):
        self.__add_round_key(state_matrix, round_key_matrix)
        self.__mix_columns_inv(state_matrix)
        self.__shift_rows_inv(state_matrix)
